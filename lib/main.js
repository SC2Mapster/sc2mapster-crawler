...
async function getLatestProjects(rootCategory, since) {
    let items = (new project_1.ProjectListItemScrapper()).process(await get('/' + rootCategory + '?filter-sort=2'));
    if (since) {
        items = items.filter((item) => {
            return item.updatedAt > since;
        });
...
async function getProjectImages(projectName) {
    return (new project_1.ProjectImageScrapper()).process(await get(`/projects/${projectName}/images`));
}
exports.getProjectImages = getProjectImages;
function findMatchingFileImage(label, images) {
    const imMap = new Map();
...
exports.findMatchingFileImage = findMatchingFileImage;
async function getForumThread(threadUrl) {
    const uinfo = url.parse(threadUrl);
    const r = (new project_1.ForumThreadScrapper()).process(await get(uinfo.path));
    r.url = threadUrl;
    return r;
}
