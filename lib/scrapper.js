...
(function (ProjectSections) {
    ProjectSections["Assets"] = "assets";
    ProjectSections["Maps"] = "maps";
})(ProjectSections = exports.ProjectSections || (exports.ProjectSections = {}));
;
//
//
function parsePager($) {
...
function parseProjectFile($) {
    ...
    pfile.downloads = Number(detailsMap.get('Downloads').text().replace(',', '').trim());
...
function parseProjectImage($) {
    ...
    page.images = [];
    const $items = $.find('.listing-attachment >li');
    $items.each((index) => {
        const imItem = {};
        const $el = $items.eq(index);
...
class MapsterConnection {
    ...
    getProjectsList(section, pageHandler) {
        return __asyncGenerator(this, arguments, function* getProjectsList_1() {
            let cpage = 1;
            while (1) {
                const $ = yield __await(this.get(`/${section}?filter-sort=2&page=${cpage}`));
    ...
    getProjectFilesList(projectName) {
        return __asyncGenerator(this, arguments, function* getProjectFilesList_1() {
            ...
            while (1) {
                ...
                const pageInfo = parsePager($.find('.listing-header'));
                yield __await(yield* __asyncDelegator(__asyncValues(parseProjectFilesList($))));
                if (cpage >= pageInfo.total)
                    break;
                ++cpage;
            }
    ...
    async getForumThread(threadUrl) {
